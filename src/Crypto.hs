...
phi :: Int -> Int
phi m = lenght[x | x <- [1..m], gcd m x == 1]
...
modPow a k m
  | k`mod`2 == 0 = ((a^2)`mod`m)^j`mod`m
  | otherwise    = (a*((((a^2)`mod`m)^j)`mod`m))`mod`m
  where 
    j = k`div`2
...
-- | Helper function is called "incrementer". 
-- |It increments a arbitrary variable "b" and uses recursion to increase b by one.
smallestCoPrimeOf :: Int -> Int
smallestCoPrimeOf a = incrementer a 2
  where incrementer :: Int -> Int -> Int 
  incrementer a b
    | gcd a b == 1 = b
    | otherwise    = incrementer a (b+1)
